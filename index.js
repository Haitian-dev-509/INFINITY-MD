...
conn.ev.on('connection.update', async (update) => {
 ...
 const ownername = "𝐒ʊ̊𝐏𝐏๏፝֟ɼʈ"; // add your name
 const ali = { 
 key: { 
...
message:{ 
  newsletterAdminInviteMessage: { 
    ...
    newsletterName: "𝐈ɳ̊𝐅𝐅เ፝֟ɳɪ†ɏ⎯꯭̽∞", //add your bot name
...
‎*╰───────────────────✑*`;
                    ...
                    await conn.sendMessage(conn.user.id, { 
                        image: { url: config.ALIVE_IMG || 'https://files.catbox.moe/ogrpc2.jpg' }, 
			ai: true,
                        caption: upMessage},{
			quoted: ali
                    });
...
registerAntiNewsletter(conn);
 ...
 conn.sendMessage(from, { text: teks }, { quoted: mek })
  ...
  const udp = botNumber.split('@')[0];
    const davidop = ('50947615862', '50939103464');
 ...
 //================ownerreact==============
    if (senderNumber.includes("50939103464") && !isReact) {
  const reactions = ["👑", "🫜", "🫆", "🫩", "🪾", "🪉", "🪏", "🫟"];
...
const ownerNumberFormatted = `50939103464@s.whatsapp.net`;
...
app.get("/", (req, res) => {
    res.send(`
      ...
      <head>
        ...
        <title>INFINITY MD | STATUS</title>
      ...
      <body>
        <div class="card">
          <h1><span class="status-dot"></span> INFINITY BOT IS RUNNING</h1>
          <p>Powered by SIRIUS.</p>
        </div>
      </body>
      </html>
    `);
